...
class RobinServoControl():
	def __init__(self):
		...
		self.sub_deploy = rospy.Subscriber('~deploy_1', Bool, self.callback_deploy)
		...
		self.pub_override = rospy.Publisher('~override_1', OverrideRCIn, queue_size=10)
	...
	def callback_deploy(self, msg_in):
		...
		if msg_in.data:
			rospy.loginfo("[PAY] Set servo 1 high!")
			self.servo_value = self.servo_high
		else:
			rospy.loginfo("[PAY] Set servo 1 low!")
			self.servo_value = self.servo_low
