...
class RobinServoControl():
	def __init__(self):
		...
		self.sub_deploy = rospy.Subscriber('~deploy_2', Bool, self.callback_deploy)
		...
		self.pub_override = rospy.Publisher('~override_2', OverrideRCIn, queue_size=10)
	...
	def callback_deploy(self, msg_in):
		...
		if msg_in.data:
			rospy.loginfo("[PAY] Set servo 2 high!")
			self.servo_value = self.servo_high
		else:
			rospy.loginfo("[PAY] Set servo 2 low!")
			self.servo_value = self.servo_low
